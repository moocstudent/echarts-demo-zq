 ...
 * version1.02 假设传入 names 为复数,获取其中数据则为[names[0],names[1],...]的形式
 *
 * @param names 传入要获取的name(最终是该name的数组)
 * @param json 总体json数据
 ...
 */
function getParamValuesByNames(names, json, key, value) {
    var ret = [];
    var len = Object.keys(json).length;
    ...
    for (var i = 0; i < len; i++) {
        if (json[i][key] == value) {
            ...
            for (var j = 0; j < names.length; j++) {
                if (j != names.length - 1) {
                    smallData += json[i][names[j]] + ',';
                } else {
                    smallData += json[i][names[j]];
                }
            }
            var formatSmallData = '[' + smallData + ']';
            smallArray.push(formatSmallData);
            ret.push(formatSmallData);
        }
    }
    return '['+ret+']';
}
